...
IndexStmt scheduleSpMMCPU_test(IndexStmt stmt, Tensor<double> B, int num) {
...
int main(int argc, char* argv[]) {
 ...
 for(int num =0; num < NUMSCHEDULES; num ++)
 {
  ...
  try{
        stmt = scheduleSpMMCPU_test(stmt,B,num);
          A.compile(stmt);
          A.assemble();
          A.compute();
  }
  ...
  double * A_ptr = (double *) A.getStorage().getValues().getData();


  bool flag = false;
  for(int i = 0; i < NUM_I * NUM_J; i ++)
  {
          if(A_ptr[i] != expected_ptr[i])
          {
                  std::cout << i << std::endl;
                  flag = true;
                  break;
