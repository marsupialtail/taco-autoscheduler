...
IndexStmt scheduleSpMMCPU_test(IndexStmt stmt, Tensor<double> A, IndexExpr precomputedExpr, int num) {
...
int main(int argc, char* argv[]) {
  ...
  for (int jj = 0; jj < NUM_J; jj++) {
      float rand_float = (float)rand()/(float)(RAND_MAX);
      if (rand_float < SPARSITY) {
        x.insert({jj}, (double)((int) (rand_float*3/SPARSITY)));
      }
  }
  A.pack();
  x.pack();
 ...
 IndexStmt stmt = C.getAssignment().concretize();
  ...
  try{
        stmt = scheduleSpMMCPU_test(stmt,A,precomputed,num);
          C.compile(stmt);
          C.assemble();
           C.compute();
  }
